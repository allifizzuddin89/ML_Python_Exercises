...
def graph_data(stock):
    ...
    #date conversion from unix timestamp to daytime matplotlib readable 
    dateconv = np.vectorize(dt.datetime.fromtimestamp)
    date = dateconv(date)

    # df = pd.DataFrame(stock_data, header=0, names=['date','open','high','low','close','adjusted_close','volume'])
    # print('\nData Frame',df.head(5))
